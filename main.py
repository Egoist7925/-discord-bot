...
def add_youtube_stream_support(opts):
    """
    GitHub Actionsなどのクラウド環境向けYouTube音源設定。
    mwebクライアント + bgutil HTTP PO Token Providerを使用する。
    Providerはsibyl.ymlで localhost:4416 に起動する。
    """
    opts["extractor_args"] = {
        "youtube": {
            "player_client": ["mweb"],
